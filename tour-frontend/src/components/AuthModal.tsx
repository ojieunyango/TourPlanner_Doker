...
export default function AuthModal({ 
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      const response = await axios.post(`${API_BASE_URL}/api/users/login`,//'http://localhost:8080/api/users/login',
         {
  ...
  const handleSignup = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await axios.post(`${API_BASE_URL}/api/users/register`,//'http://localhost:8080/api/users/register', 
        newUser, {
        headers: {
          'Content-Type': 'application/json',
        },
